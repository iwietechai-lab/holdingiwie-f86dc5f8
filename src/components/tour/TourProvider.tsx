...
interface TourContextType {
  ...
  hasCompletedCurrentTour: () => boolean;
  resetTours: () => void;
  triggerTourAfterVerification: () => void;
}
...
export const TourProvider = ({ children }: TourProviderProps) => {
  ...
  const startTour = useCallback((pageKey?: string) => {
    ...
    console.log('TourProvider - Starting tour for:', key, 'Available:', !!tourConfigs[key]);
    if (tourConfigs[key]) {
      hasInitialized.current = false;
      setCurrentStepIndex(0);
      setIsActive(true);
      sessionStorage.removeItem(TOUR_TRIGGER_KEY);
      console.log('TourProvider - Tour started, isActive set to true');
    }
  }, [getCurrentPageKey]);
  ...
  const resetTours = useCallback(() => {
    ...
    setCompletedTours([]);
    console.log('TourProvider - All tours reset');
  }, []);
...
// Export for external use (like FacialVerificationGuard)
